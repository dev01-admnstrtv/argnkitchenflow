...
import { criarAgrupamento } from '@/lib/actions/produtos'
...
export default function NovoAgrupamentoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultado = await criarAgrupamento(formDataToSend)
      
      if (resultado.success) {
        router.push('/agrupamentos')
      } else {
        setErrors({ submit: resultado.error || 'Erro ao criar agrupamento' })
      }
    } catch (error) {
      console.error('Erro ao criar agrupamento:', error)
      setErrors({ submit: 'Erro inesperado ao criar agrupamento' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-3xl font-bold">Novo Agrupamento</h1>
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end space-x-4 pt-4">
                ...
                <Button type="submit" disabled={loading}>
                  {loading ? 'Criando...' : 'Criar Agrupamento'}
                </Button>
              </div>
            </CardContent>
          </Card>
